...
class GlassVerifyPhoneScreen extends ConsumerStatefulWidget {
  final String phone;

  const GlassVerifyPhoneScreen({super.key, required this.phone});

  @override
  ConsumerState<GlassVerifyPhoneScreen> createState() => _GlassVerifyPhoneScreenState();
}

class _GlassVerifyPhoneScreenState extends ConsumerState<GlassVerifyPhoneScreen> {
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('${ApiConfig.baseUrl}/api/v2/auth/verify-phone'),
        headers: {
      ...
      if (response.statusCode == 200) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Телефон подтверждён!'),
  ...
  Future<void> _resendCode() async {
    ...
    try {
      ...
      await http.post(
        Uri.parse('${ApiConfig.baseUrl}/api/v2/auth/verify-phone/resend'),
        headers: {
          'Content-Type': 'application/json',
          if (token != null) 'Authorization': 'Bearer $token',
        },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('Подтверждение телефона', style: LiquidGlassTextStyles.h3Light(isDark)),
      ),
      body: Padding(
