...
const CONFIG = {
  ...
  totalUsers: 50, // Увеличиваем количество пользователей
  concurrentUsers: 8, // Больше одновременных пользователей
  testDuration: 1200, // 20 минут
  userDelay: 5000, // 5 секунд между пользователями
  actionDelay: 1500, // 1.5 секунды между действиями
  databaseCheckInterval: 30000 // Проверка базы каждые 30 секунд
};
...
// РЕАЛЬНЫЙ ПОЛЬЗОВАТЕЛЬ - нажимает кнопки как человек + API вызовы
async function simulateRealUser(userId) {
  ...
  try {
    ...
    if (userStats.role === 'client') {
      ...
      // 5.2 СОЗДАНИЕ ЗАКАЗА - УСЛОЖНЕННЫЙ СЦЕНАРИЙ
    ...
    if (userStats.role === 'master') {
      ...
      // 6.2 ЗАГРУЗКА ВИДЕО - УСЛОЖНЕННЫЙ СЦЕНАРИЙ
...
// Запуск УСЛОЖНЕННОГО РЕАЛЬНОГО UI теста
async function runEnhancedRealUITest() {
  console.log('🚀 STARTING ENHANCED REAL UI LOAD TEST FOR MEBELPLACE');
  ...
  console.log('\n' + '═'.repeat(80));
  console.log('📊 ENHANCED REAL UI LOAD TEST RESULTS');
  console.log('═'.repeat(80));
...
// Запуск
runEnhancedRealUITest().catch(console.error);
