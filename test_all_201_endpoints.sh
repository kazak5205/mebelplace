...
echo -e "${BLUE}🧪 ПОЛНОЕ ТЕСТИРОВАНИЕ ВСЕХ 201 ENDPOINTS${NC}"
echo "=============================================="
...
# Пользователь 3 - админ (если возможно)
USER3_RESPONSE=$(curl -s -X POST https://mebelplace.com.kz/api/v2/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "email_or_phone": "+7700'$(date +%s)'",
    "password": "TestPassword123!",
...
USER3_TOKEN=$(echo $USER3_RESPONSE | jq -r '.access_token')
USER3_ID=$(echo $USER3_RESPONSE | jq -r '.user.id')

echo -e "${GREEN}✅ Пользователи созданы:${NC}"
echo "👤 User1 (ID: $USER1_ID): ${USER1_TOKEN:0:30}..."
echo "👤 User2 (ID: $USER2_ID): ${USER2_TOKEN:0:30}..."
echo "👤 User3 (ID: $USER3_ID): ${USER3_TOKEN:0:30}..."
...
check_endpoint "POST" "/gamification/award-points" '{"user_id":'$USER1_ID',"points":100,"reason":"Test"}' "Начислить очки" true "$USER3_TOKEN"
...
check_endpoint "GET" "/moderation/videos" "" "Получить видео для модерации" true "$USER3_TOKEN"
check_endpoint "POST" "/moderation/videos/1/approve" "" "Одобрить видео" true "$USER3_TOKEN"
check_endpoint "POST" "/moderation/videos/1/reject" "" "Отклонить видео" true "$USER3_TOKEN"
check_endpoint "GET" "/moderation/comments" "" "Получить комментарии для модерации" true "$USER3_TOKEN"
check_endpoint "POST" "/moderation/comments/1/approve" "" "Одобрить комментарий" true "$USER3_TOKEN"
check_endpoint "POST" "/moderation/comments/1/reject" "" "Отклонить комментарий" true "$USER3_TOKEN"
check_endpoint "GET" "/moderation/streams" "" "Получить стримы для модерации" true "$USER3_TOKEN"
check_endpoint "GET" "/moderation/reports" "" "Получить жалобы" true "$USER3_TOKEN"
...
check_endpoint "GET" "/analytics/platform" "" "Получить аналитику платформы" true "$USER3_TOKEN"
check_endpoint "GET" "/analytics/engagement" "" "Получить аналитику вовлеченности" true "$USER3_TOKEN"
...
check_endpoint "GET" "/admin/support/tickets" "" "Получить все тикеты поддержки" true "$USER3_TOKEN"
check_endpoint "GET" "/admin/support/tickets/1" "" "Получить тикет поддержки" true "$USER3_TOKEN"
check_endpoint "POST" "/admin/support/tickets/1/replies" '{"message":"Admin reply"}' "Ответить на тикет от имени админа" true "$USER3_TOKEN"
check_endpoint "GET" "/admin/users" "" "Получить всех пользователей" true "$USER3_TOKEN"
check_endpoint "POST" "/admin/users/$USER1_ID/ban" '{"reason":"Test ban"}' "Забанить пользователя" true "$USER3_TOKEN"
check_endpoint "POST" "/admin/users/$USER1_ID/unban" "" "Разбанить пользователя" true "$USER3_TOKEN"
check_endpoint "PUT" "/admin/users/$USER1_ID/role" '{"role":"moderator"}' "Изменить роль пользователя" true "$USER3_TOKEN"
...
check_endpoint "POST" "/notifications/broadcast" '{"title":"Test Broadcast","message":"Test message"}' "Отправить broadcast уведомление" true "$USER3_TOKEN"
check_endpoint "GET" "/notifications/templates" "" "Получить шаблоны уведомлений" true "$USER3_TOKEN"
