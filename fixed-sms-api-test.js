...
// 🚀 РАСШИРЕННЫЙ ТЕСТ С ПОЛНЫМИ СЦЕНАРИЯМИ - 500 ПОЛЬЗОВАТЕЛЕЙ
const config = {
...
console.log('🚀 EXPANDED FULL SCENARIO TEST - 500 USERS WITH COMPLETE INTERACTIONS');
console.log('════════════════════════════════════════════════════════════════════════════════');
...
console.log(`🖱️  Real UI: ENABLED (SMS из API + полные сценарии + все взаимодействия)`);
...
// Функция для создания пользователя с правильным получением SMS
async function createUserWithCorrectSMS(userId) {
  ...
  try {
    ...
    // Ждем SMS код из API ответа
    console.log(`📱 User ${userId}: Ждет SMS код из API...`);
    ...
    page.on('response', async response => {
      if (response.url().includes('/api/auth/send-sms-code') && response.status() === 200) {
        try {
          ...
          console.log(`❌ User ${userId}: Ошибка парсинга API ответа - ${error.message}`);
        }
      }
    });
    ...
    // Если SMS код не получен, пробуем получить его напрямую через API
    if (!smsCode) {
      console.log(`🔍 User ${userId}: SMS код не получен из браузера, пробуем API напрямую...`);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.success && result.code) {
            smsCode = result.code;
            console.log(`📱 User ${userId}: Получен SMS код через прямой API: ${smsCode}`);
          }
        }
      } catch (error) {
        console.log(`❌ User ${userId}: Ошибка прямого API запроса - ${error.message}`);
      }
    }
    ...
    // Теперь выполняем полные взаимодействия
    if (userRole === 'client') {
      await executeFullClientInteractions(page, userId);
    } else {
      await executeFullMasterInteractions(page, userId);
    }
    
    console.log(`🎉 User ${userId}: Завершил полные взаимодействия!`);
    completedUsers++;
...
// Полные взаимодействия для клиентов
async function executeFullClientInteractions(page, userId) {
  console.log(`👤 User ${userId}: Начинает ПОЛНЫЕ взаимодействия КЛИЕНТА`);
  ...
  // 5. ДОПОЛНИТЕЛЬНЫЕ ВЗАИМОДЕЙСТВИЯ КЛИЕНТА
  console.log(`👤 User ${userId}: Дополнительные взаимодействия`);
  
  try {
    // Просмотр видео мастеров
    await page.goto(`${config.targetUrl}/videos`, { waitUntil: 'networkidle' });
    console.log(`👤 User ${userId}: Смотрит видео мастеров`);
    successfulActions++;
    totalActions++;
    
    await page.waitForTimeout(config.delayBetweenActions);
    
    const videoCards = page.locator('.video-card, [data-testid="video-card"], .video-item');
    ...
    if (videoCount > 0) {
      for (let i = 0; i < Math.min(3, videoCount); i++) {
        try {
          ...
          await page.waitForLoadState('networkidle');
          console.log(`👤 User ${userId}: Смотрит видео ${i + 1}`);
          successfulActions++;
          totalActions++;
          
          await page.waitForTimeout(config.delayBetweenActions);
          
          // Ставим лайки на видео
          const likeButtons = page.locator('button:has-text("❤"), .like-btn, [data-testid="like"]');
          const likeCount = await likeButtons.count();
          if (likeCount > 0) {
            for (let j = 0; j < Math.min(2, likeCount); j++) {
              try {
                await likeButtons.nth(j).click();
                console.log(`👤 User ${userId}: Поставил лайк на видео ${i + 1}`);
                successfulActions++;
                totalActions++;
                globalData.likes.push({ clientId: userId, videoIndex: i, likeIndex: j });
          ...
          // Комментируем видео
          const commentInputs = page.locator('input[placeholder*="комментарий"], textarea[placeholder*="комментарий"]');
          const commentCount = await commentInputs.count();
          if (commentCount > 0) {
            for (let k = 0; k < Math.min(2, commentCount); k++) {
              try {
                const comments = [
                  `Отличное видео! Очень профессионально!`,
                  `Интересно, сколько стоит такая работа?`,
                  `Красиво получилось! Хочу заказать.`,
                  `Профессионально! Рекомендую всем!`,
                  `Отличная работа! Видно опыт мастера.`
                ];
                const randomComment = comments[Math.floor(Math.random() * comments.length)];
                ...
                const submitCommentButtons = page.locator('button:has-text("Отправить"), button:has-text("Comment")');
                if (await submitCommentButtons.count() > 0) {
                  await submitCommentButtons.nth(0).click();
                  console.log(`👤 User ${userId}: Оставил комментарий на видео ${i + 1}`);
                  successfulActions++;
                  totalActions++;
                  globalData.comments.push({ clientId: userId, videoIndex: i, comment: randomComment });
    ...
    // Просмотр своих заказов и взаимодействие с ними
    await page.goto(`${config.targetUrl}/user/orders`, { waitUntil: 'networkidle' });
    console.log(`👤 User ${userId}: Просматривает свои заказы`);
    successfulActions++;
    totalActions++;
    ...
    const editButtons = page.locator('button:has-text("Редактировать"), button:has-text("Edit"), .edit-btn');
    const editCount = await editButtons.count();
    
    if (editCount > 0) {
      for (let i = 0; i < Math.min(2, editCount); i++) {
        try {
          await editButtons.nth(i).click();
          await page.waitForLoadState('networkidle');
          console.log(`👤 User ${userId}: Редактирует заказ ${i + 1}`);
          successfulActions++;
          totalActions++;
          ...
          const descInput = page.locator('textarea[name*="description"], textarea[placeholder*="описание"]');
          if (await descInput.count() > 0) {
            await descInput.nth(0).fill(`Обновленное описание заказа от клиента ${userId}. Добавлены новые требования.`);
            console.log(`👤 User ${userId}: Обновил описание заказа ${i + 1}`);
            successfulActions++;
            totalActions++;
          ...
          // Сохраняем изменения
          const saveButton = page.locator('button:has-text("Сохранить"), button:has-text("Save")');
          if (await saveButton.count() > 0) {
            await saveButton.nth(0).click();
            console.log(`👤 User ${userId}: Сохранил изменения заказа ${i + 1}`);
            successfulActions++;
            totalActions++;
    ...
    await page.goto(`${config.targetUrl}/notifications`, { waitUntil: 'networkidle' });
    ...
    const markReadButtons = page.locator('button:has-text("Отметить как прочитанное"), button:has-text("Mark as read")');
    const markReadCount = await markReadButtons.count();
    
    if (markReadCount > 0) {
      for (let i = 0; i < Math.min(3, markReadCount); i++) {
        try {
          await markReadButtons.nth(i).click();
          console.log(`👤 User ${userId}: Отметил уведомление ${i + 1} как прочитанное`);
          successfulActions++;
          totalActions++;
    ...
    console.log(`❌ User ${userId}: Ошибка дополнительных взаимодействий - ${error.message}`);
    failedActions++;
    totalActions++;
  }
}

// Полные взаимодействия для мастеров
async function executeFullMasterInteractions(page, userId) {
  console.log(`👤 User ${userId}: Начинает ПОЛНЫЕ взаимодействия МАСТЕРА`);
...
async function runLoadTest() {
  console.log('🚀 Запуск исправленного теста с правильным получением SMS...');
  ...
  for (let i = 1; i <= config.totalUsers; i++) {
    ...
    createUserWithCorrectSMS(i).catch(error => {
